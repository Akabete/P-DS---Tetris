...
void draw_tile(int x, int y, int color){
    int x1 = board.offset_x + (x * board.tile_size);
    int y1 = board.offset_y + (y * board.tile_size);

    int x2 = x1 + board.tile_size;
    int y2 = y1 + board.tile_size;

    gfx_filledRect(x1, y1, x2, y2, color);
}

void draw_walls(){
    int left_top_x = board.offset_x;
    int left_bottom_x = left_top_x;
    int left_top_y = board.offset_y;
    int left_bottom_y = left_top_y + (BOARD_HEIGHT * board.tile_size);
    gfx_line(left_top_x, left_top_y, left_bottom_x, left_bottom_y, get_board_color(WALL_VALUE));


    int right_top_x = board.offset_x + (BOARD_WIDTH * board.tile_size);
    int right_bottom_x = right_top_x;
    int right_top_y = board.offset_y;
    int right_bottom_y = right_top_y + (BOARD_HEIGHT * board.tile_size);
...
void draw_board(){
    for(int y = 0; y < BOARD_HEIGHT; y++){
        ...
        int board_value = board.grid[y][x];
...
void draw_active_piece(){
    for(int r = 0; r < PIECE_SIZE; r++){
        for(int c = 0; c < PIECE_SIZE; c++){
            ...
            if(value != 0)
                draw_tile(active_piece.x + c, active_piece.y + r, get_board_color(value));
        }
    }
}
...
void draw_next_piece(){
    for(int r = 0; r < PIECE_SIZE; r++){
        for(int c = 0; c < PIECE_SIZE; c++){
            int value = pieces[next_piece.type][0][r][c];
            if(value != 0)
